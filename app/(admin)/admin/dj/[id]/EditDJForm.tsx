...
interface Props {
  dj: {
    ...
    bio: string;
    image: string;
    featured: boolean;           // ← Make sure this is passed
    socialLinks?: {
      instagram?: string;
      soundcloud?: string;
      twitter?: string;
          ...
          <CardContent className="space-y-4">
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-2">
                <Label>Name</Label>
                <Input name="name" defaultValue={dj.name} required />
              </div>
              <div className="space-y-2">
                <Label>Slug</Label>
                <Input name="slug" defaultValue={dj.slug} required />
              </div>
            </div>

            <div className="space-y-2">
              <Label>Genre</Label>
              <Input name="genre" defaultValue={dj.genre} required />
            </div>

            <div className="space-y-2">
              <Label>Biography</Label>
              <Textarea name="bio" rows={6} defaultValue={dj.bio} required />
            </div>

            <div className="space-y-2">
              <Label>Image URL</Label>
              <Input name="image" type="url" defaultValue={dj.image} required />
            </div>
          </CardContent>
        </Card>
